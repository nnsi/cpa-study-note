...
| **構成の一貫性** | AIが勝手に違う構成で書いて手戻り | 13機能作っても構成の一致率95% |
...
| バックエンド: コアファイル全体（route/usecase/repository/index） | 59/62 = **95.4%** |
| フロントエンド: api.ts + index.ts 保有率 | 15/15 = **100%** |
| フロントエンド: コアファイル全体（api/hooks/components/index） | 57/60 = **94.3%** |
...
| 指標 | 数値 |
|------|------|
| テストファイル数 | 56（ユニット50 + E2E 6） |
| テストコード行数 | 約19,700行 |
| テスト比率 | 全TypeScriptコードの**37.8%** |
...
*なお、この記事のブラッシュアップには5人のWebエンジニアペルソナ（シニア、若手、CTO、技術記事ライター、懐疑派）を設定し、それぞれの視点でレビューしてもらう手法を使った。特に懐疑派の「その数字は本当か？」「隠しているコストはないか？」という視点が、記事の信頼性を高めるのに役立った。AIと一緒に記事を書く場合、このような多角的なセルフレビューは有効かもしれない。*