...
### 6.4 ツリーエディタUI
- [x] `/domains/:id/subjects/:subjectId` ルート作成
- [x] `TreeEditor.tsx` - コンテナコンポーネント
- [x] `TreeNode.tsx` - 再帰的ノード表示
- [x] `TreeNodeEditor.tsx` - インライン編集（TreeNode内に統合）
- [x] `TopicDetailEditor.tsx` - 論点詳細編集パネル
- [x] `AddNodeButton.tsx` - 追加ボタン（TreeNode/TreeEditor内に統合）
- [x] `useTreeState.ts` - 状態管理フック
- [ ] `useTreeDragDrop.ts` - D&Dロジック（自前実装）※後回し
- [x] 一括保存ボタン・処理

### 6.5 CSVインポートUI
- [x] CSVインポートボタン
- [x] ファイル選択ダイアログ
- [x] インポート結果表示（成功数、エラー一覧）

### 6.6 新規ユーザーサンプル作成
- [x] `apps/api/src/features/auth/sample-data.ts` 作成
- [x] `createSampleDataForNewUser` 関数実装
- [x] `handleOAuthCallback` での呼び出し追加
- [x] サンプルデータ内容確定

### 6.7 ルーティング更新
- [x] `routeTree.gen.ts` 更新
- [x] ヘッダーナビゲーション更新
- [x] 既存 `/subjects` ルートからのリダイレクト
...
### 7.2 既存機能との統合
- [x] チャット機能がユーザー所有のトピックで動作
- [x] ノート機能がユーザー所有のトピックで動作
- [ ] 進捗表示がユーザー所有のデータで動作
...
- [x] 型エラーがゼロ
- [x] ユーザーが学習領域を作成できる
- [x] ユーザーが科目を追加・編集・削除できる
- [x] ツリーエディタで単元・論点を編集できる
- [x] CSVインポートが動作する
- [x] 新規ユーザーにサンプルデータが作成される（実装済み）
- [x] 論理削除が正しく動作する
- [x] 既存のチャット・ノート機能が正常に動作する
