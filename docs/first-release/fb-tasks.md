...
- [x] `findFilteredTopics()` の `lastChatAt` を `max(chatMessages.createdAt)` に変更
  - 対象: `apps/api/src/features/topic/repository.ts`
- [x] 変更後、復習フィルタの `daysSinceLastChat` / `hasPostCheckChat` が正しく動作することを確認

### 2. 日次メトリクスを表示可能にする

**方針**: スナップショットを廃止し、オンザフライ集計に変更（運用がシンプル、D1の性能で十分）

- [x] `GET /metrics/daily` をオンザフライ集計に変更
  - 対象: `apps/api/src/features/metrics/usecase.ts`, `repository.ts`
- [x] `checkedTopicCount` を `topicCheckHistory` から「日付時点の状態」を正確に復元する計算に修正
...
- [x] `DailyMetricsChart` で `goodQuestionCount`（軸B）を表示
...
- [x] `users` テーブルに `timezone` カラムを追加（デフォルト: `"Asia/Tokyo"`）
  - 対象: `packages/db/src/schema/users.ts`
- [x] マイグレーション作成・適用
- [x] 初回ログイン時にブラウザの `Intl.DateTimeFormat().resolvedOptions().timeZone` を取得して保存
  - 対象: `apps/api/src/features/auth/usecase.ts`（Google OAuth コールバック）
- [x] 集計時にユーザーの timezone を使って日付境界を計算
  - 対象: `apps/api/src/features/metrics/repository.ts`
...
- [x] `listSessionsByTopic` の集計を SQL に寄せる
  - 対象: `apps/api/src/features/chat/usecase.ts`
...
- [x] ノート詳細画面で `keyPoints` / `stumbledPoints` の編集を可能にする
  - 対象: `apps/web/src/routes/notes/$noteId.tsx`
- [x] API は既に対応済み（`apps/api/src/features/note/route.ts`）
