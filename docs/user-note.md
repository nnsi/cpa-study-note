...
- [ ] API: `POST /api/notes` の独立ノート作成を実装
- [ ] API: バリデーション（本文/配列/長さ制限）を追加
- [ ] API: レスポンスに `source` を含める（または `sessionId` から判定）
- [ ] UI: Topic Detail のノートタブに作成導線を追加
- [ ] UI: 独立ノート作成モーダルを実装
- [ ] UI: ノート一覧に「手動/チャット」ラベルを表示
- [ ] UI: チャット由来のみ再生成ボタンを表示
- [ ] テスト: ユニットテスト（usecase/logic）
- [ ] テスト: 統合テスト（API/DB）
- [ ] テスト: 独立ノート作成/表示/更新の最低限E2E
