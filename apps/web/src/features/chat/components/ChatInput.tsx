...
export const ChatInputView = ({
  ...
  return (
    <>
      <div className="border-t border-ink-100 bg-white/80 backdrop-blur-sm px-2 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] lg:px-4 lg:pt-3 lg:pb-3">
        {/* 画像プレビュー */}
        {imageId && (
        ...
        <div className="flex gap-1 items-center">
          ...
          {/* 音声テキスト補正ボタン */}
          {content.trim() && onCorrectSpeech && (
            <button
              type="button"
              onClick={onCorrectSpeech}
              disabled={isSubmitting || isCorrectingSpeech}
              className="flex-shrink-0 p-1 text-ink-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed"
              title="AIでテキストを補正"
            >
              {isCorrectingSpeech ? (
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder={
                imageId ? "この画像について質問..." : "Shift+Enterで改行"
              }
